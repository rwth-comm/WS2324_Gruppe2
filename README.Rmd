...
ggplot(df) +
 ...
 title = "Stichprobe", subtitle = paste0("Altersverteilung im Histogramm (n=",nrow(df), ")") )+
 theme_minimal()
...
jmv::linReg(data = df, 
            ...
            blocks = c("orientation1_5", "orientation1_7","orientation1_3", "orientation1_1", 
...
df$konsumgroup <- cut(df$SMK, labels = c("hoch", "niedrig"), breaks = c(-Inf, median(df$SMK), Inf))
...
df %>% 
  recode(df$PO,
  "orientation1_2" = "rechts",
  "orientation1_5" = "rechts",
  "orientation1_1" = "links",
  "orientation1_3" = "links",
  .default = NA_character_)
  filter(PO > 3) %>% 
  droplevels() %>%
  t.test(df$PO == "rechts"$PBE,
       (df$PO == "links"$PBE))
