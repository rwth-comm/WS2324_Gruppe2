...
jmv::linReg(data = df, 
            ...
            blocks = c("orientation1_1", "orientation1_2","orientation1_3", "orientation1_4", 
                       "orientation1_5", "orientation1_6", "orientation1_7", "orientation1_8"),
...
df$SMK <- cut(df$SMK, labels = c("niedrig", "hoch"), breaks = c(-Inf, median(df$SMK), Inf))

t.test(filter(df, df$SMK == "niedrig")$SMK,
       filter(df, df$SMK == "hoch")$SMK)
...
```{r}
jmv::mancova(data = df, factors = c("gender", "urban", "ef"), deps = c("PBE", "BEK"), covs = c(age), multivar = list("pillai", "wilks", "hotel", "roy"))
```
