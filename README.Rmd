...
ggplot() +
  ...
  labs(
    ...
    y = "persönliches Bedrohungsempfinden (1- 6)",
...
ggplot() +
  ...
  labs(
    x = "Ernährungsform",
    y = "Bereitschaft zum Ergreifen von Klimaschutzmaßnahmen (1- 6)",
    title = "T-Test zur Unterschiedshypothese",
...
df %>% 
  ...
  jmv::ancova(dep = PBE, 
               covs = age,
               factors = c("urban", "ef")) -> he1
df %>% 
  filter(ef != "vegan") %>% 
  droplevels() %>%
  jmv::ancova(dep = BEK, 
               covs = age,
               factors = c("urban", "ef")) -> he2

df %>% 
  filter(gender != "rather not say") %>% 
  droplevels() %>%
  jmv::ancova(dep = diff_pref, 
              covs = age,
               factors = c("gender", "jobtypegroup"),
             emmPlots = TRUE, emmTables = TRUE, emMeans = list(c("gender"), c("jobtypegroup"),
                                                                c("gender", "jobtypegroup"),
                                                                c("jobtypegroup", "gender"))) -> mancova2

mancova1he1 <- mancova1$emm[[1]]$emmTable$asDF
mancova1he2 <- mancova1$emm[[2]]$emmTable$asDF
mancova1ie <- mancova1$emm[[3]]$emmTable$asDF

mancova2he1 <- mancova2$emm[[1]]$emmTable$asDF
mancova2he2 <- mancova2$emm[[2]]$emmTable$asDF
mancova2ie <- mancova2$emm[[3]]$emmTable$asDF
